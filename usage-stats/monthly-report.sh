...
#awk -F, '{ if ($2 > 0) print $1 }' all/corehours-${SYEAR}-${SMONTH}-user-all.csv | head
awk -F, '{ if ($2 > 0) print $1 }' all/corehours-${SYEAR}-${SMONTH}-user-all.csv | sort -u > activeusers-UIDs-${SYEAR}-${SMONTH}.txt
